...
class Uri implements UriInterface
{
    ...
    public function __toString()
    {
        ...
        if (empty($this->scheme) == false) {
            $result_string .= $this->scheme . ':';
        }

        if (empty($this->authority) == false) {
            $result_string .= '//' . $this->authority;
        }

        if (empty($this->path) == false) {
            $is_rootless = substr($this->path, 0, 1) != '/';
            if ($is_rootless && !empty($this->authority)) {
                $result_string .= '/' . $this->path;
            }
            else if ($is_rootless == false && substr($this->path, 0, 2) == '//') {
        ...
        if (empty($this->query) == false) {
            $result_string .= '?' . $this->query;
        }

        if (empty($this->fragment) == false) {
            $result_string .= '#' . $this->fragment;
        }
